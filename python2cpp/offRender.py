...
from Locator_Vicon import Locator_vicon
import pyrr
import trimesh
import numpy as np
import pyglet
...
def offRender():
    ...
    while True:
        ...
        try:
            ...
            scene.set_pose(node=cam_node, pose=createCamPos(x, y, z))
        ...
        frame = deformation_curve(20, 8, math.pi / 2, (x, y), color)
...
def createCamPos(x=0.5, y=0, z=0.4):
    ...
    # x_d = 0 - x_d
    print(y, -x, z)
    cam_pose = pyrr.matrix44.create_look_at((y, -x, z), (0, 0, 0), (0, 0, 1))
    cam_pose = np.linalg.inv(cam_pose.T)
    return cam_pose
